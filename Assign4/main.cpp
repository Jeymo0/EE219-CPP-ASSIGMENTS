...
public:
    ...
    Point(double x, double y, const Color & color) : _x(x), _y(y), clr(color) { }
    void moveBy(double x, double y) { _x += x; _y += y; }
    void rotate(double angle, Point pRef);
    void draw() { cout << "not yet implemented" << endl; }
    void setColor(const Color & color) { clr = color; }
    void print() { cout << "(" << _x << "," << _y << ") "; clr.print(); }
};
...
void Point::rotate(double angle, Point pRef) {
...
public:
    ...
    Line(const Point & p1, const Point & p2, const Color & color) : _p1(p1), _p2(p2), clr(color) {}
    void moveBy(double x, double y) { _p1.moveBy(x,y); _p2.moveBy(x,y); }
    void rotate(double angle, Point pRef) { _p1.rotate(angle, pRef); _p2.rotate(angle, pRef); }
    void setColor(const Color & color) { clr = color; }
    void draw() { cout << "not yet implemented" << endl; }
    void print() {
        _p1.print();
...
public:
    ...
    Triangle(const Point & p1, const Point & p2, const Point & p3, const Color & color) :
            _p1(p1), _p2(p2), _p3(p3), clr(color) {}
    void moveBy(double x, double y) { _p1.moveBy(x,y); _p2.moveBy(x,y); _p3.moveBy(x,y);}
    void rotate(double angle, Point pRef) {
...
public:
    Canvas(int max_elements) :
            maxElements(max_elements), numPoints(0), numLines(0), numTriangles(0) {
        pArray = new Point*[maxElements];
        lArray = new Line*[maxElements];
        tArray = new Triangle*[maxElements];
    }
    void add(const Point & p) {
        if (numPoints == maxElements) return;
        pArray[numPoints++] = new Point(p);
    }
    void add(const Line & l) {
        if (numLines == maxElements) return;
        lArray[numLines++] = new Line(l);
    }
    void add(const Triangle & t) {
        if (numTriangles == maxElements) return;
...
private:
    ...
    Color clr;
public:
     Circle(const Point & p1, double const &rad) :
            _p1(p1), _rad(rad) {}
    Circle(const Point & p1,double const &rad, const Color & color) :
            _p1(p1), _rad(rad), clr(color) {}
    void moveBy(double x, double y) { _p1.moveBy(x,y); }
    void rotate(double angle, Point pRef) {
        _p1.rotate(angle, pRef);
    }
    void setColor(const Color & color) { clr = color; }
...
void testTriangle() {
    Triangle tri(Point(1,2), Point(3,4), Point(5,6), Color(0.2,0.3,0.4));
    tri.print();
}
void testCircle(){
    Circle c(Point(1,1),5,Color(1,1,1));
    c.print();
}
